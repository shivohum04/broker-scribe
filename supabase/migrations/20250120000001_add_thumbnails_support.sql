...
-- This migration adds thumbnail_urls array to store thumbnail URLs alongside original images
...
CREATE INDEX idx_properties_thumbnail_urls ON public.properties USING GIN (thumbnail_urls);

-- Update the existing properties to have empty thumbnail_urls arrays
...
-- Add storage policies for thumbnail access (same as images)
-- Thumbnails will be stored in the same bucket with -thumb.webp suffix
-- The existing policies should cover thumbnail access since they're in the same bucket
